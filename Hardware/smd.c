...
void smd_send_data(uint8_t *data, uint8_t len)
{
    ...
    {
        // 路由逻辑：根据变量决定从哪个串口发出去
        if (g_smd_target_uart == 1) 
        {
            DL_UART_Main_transmitDataBlocking(UART_MOTOR_INST, data[i]); 
        } 
        else if (g_smd_target_uart == 2) 
        {
            DL_UART_Main_transmitDataBlocking(UART_MOTOR_2_INST, data[i]); 
        }
    }
}
/**
