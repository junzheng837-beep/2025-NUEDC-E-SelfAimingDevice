...
void Key_Read(void)
{
    ...
    for(int i=0; i<KEY_Number; i++){
        switch(Key[i].Judge_State){
            case 0:
                if(Key[i].Down_State==0){ Key[i].Judge_State=1; Key[i].Down_Time=0; }
                else { Key[i].Judge_State=0; }
                break;
            case 1:
                if(Key[i].Down_State==0){ Key[i].Judge_State=2; }
                else { Key[i].Judge_State=0; }
                break;
            case 2:
...
void KEY_PROC(void)
{
    ...
    else if(Key[2].Short_Flag==1) // KEY3短按：切换页 / 切光标
    {
        if((OLED_View_Select == 2 || OLED_View_Select == 1) && Tuning_Mode == 1) {
            ...
            OLED_View_Select++;
            if(OLED_View_Select > 4) OLED_View_Select = 1; // 仅在几页数据看板之间来回切换
        }
        Key[2].Short_Flag=0;
    }
    ...
    if(Key[0].Double_Flag==1)
    {
        ...
        if (Tracking_Test_Flag) Tuning_State = 1;
        Key[0].Double_Flag=0;
    }
    ...
    else if(Key[2].Long_Flag==1)
    {
        if(OLED_View_Select == 2 || OLED_View_Select == 1) {
            Tuning_Mode = !Tuning_Mode; // 开启 / 关闭调参模式
        }
        Key[2].Long_Flag=0;
    }
}
