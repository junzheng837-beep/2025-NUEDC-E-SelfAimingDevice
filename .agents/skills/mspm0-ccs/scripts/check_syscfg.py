...
def find_validation_hints(root: Path) -> dict[str, str]:
    ...
    if makefile.exists():
        ...
        if sysconfig:
            cmd = sysconfig.group(0).strip()
            # CCS runs this from within the Debug folder, so -o "." means Debug.
            # For root CLI usage, we must fix it to -o "Debug"
            hints["sysconfig_cli"] = re.sub(r'-o\s+"?\."?', '-o "Debug"', cmd)
